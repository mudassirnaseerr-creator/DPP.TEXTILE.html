...
<style>
  :root { --eu-blue:#003399; --eu-gold:#FFCC00; --ink:#0d1b2a; --mist:#f4f6fb; --border:#dde3ef; }
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'DM Sans',sans-serif;background:var(--mist);color:var(--ink);min-height:100vh}

  /* HEADER */
  header{background:linear-gradient(135deg,#001f6b 0%,#003399 60%,#005580 100%);color:white;position:relative;overflow:hidden}
  header::before{content:'';position:absolute;inset:0;background:url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none'%3E%3Cg fill='%23ffffff' fill-opacity='0.04'%3E%3Ccircle cx='30' cy='30' r='2'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E");pointer-events:none}
  .header-inner{max-width:1100px;margin:0 auto;padding:48px 32px 40px;display:flex;align-items:center;gap:32px}
  .eu-badge{flex-shrink:0;width:80px;height:80px;background:var(--eu-gold);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:36px;box-shadow:0 8px 32px rgba(0,0,0,0.3);animation:pulse-badge 3s ease-in-out infinite}
  ...
  /* ADMIN BADGE in strip */
  .admin-status{display:flex;align-items:center;gap:10px}
  .admin-badge{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:100px;font-size:0.78rem;font-weight:600;cursor:pointer;border:none;font-family:'DM Sans',sans-serif;transition:all 0.2s}
  .admin-badge.locked{background:#fdecea;color:#b5290a}
  .admin-badge.unlocked{background:#e6f4ec;color:#2d7a4f}

  /* TOAST */
  .save-toast{position:fixed;bottom:28px;right:28px;z-index:9999;background:#1a3a1a;color:white;padding:12px 20px;border-radius:12px;font-size:0.85rem;font-weight:500;display:flex;align-items:center;gap:8px;box-shadow:0 8px 32px rgba(0,0,0,0.25);opacity:0;pointer-events:none;transform:translateY(12px);transition:all 0.3s ease}
  ...
  /* CARD */
  .cat-card{background:white;border:1.5px solid var(--border);border-radius:16px;padding:24px;cursor:pointer;transition:transform 0.18s,box-shadow 0.18s,border-color 0.18s;position:relative;overflow:hidden;animation:fadeUp 0.4s ease both}
  ...
  /* VIEW MODE (read-only) */
  .field-view-group{margin-bottom:16px;padding:12px 14px;border:1.5px solid var(--border);border-radius:10px;background:white}
  .field-view-label{font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#7a87a3;margin-bottom:4px}
  .field-view-value{font-size:0.92rem;color:var(--ink);line-height:1.5}
  .field-view-value.empty{color:#b0bace;font-style:italic;font-weight:300}
  ...
  /* PASSWORD MODAL */
  .pw-overlay{display:none;position:fixed;inset:0;background:rgba(13,27,42,0.7);backdrop-filter:blur(8px);z-index:2000;align-items:center;justify-content:center;padding:24px}
  .pw-overlay.open{display:flex}
  .pw-box{background:white;border-radius:24px;padding:40px;max-width:380px;width:100%;text-align:center;box-shadow:0 32px 80px rgba(0,0,0,0.3);animation:slideUp 0.3s ease}
  .pw-icon{font-size:2.5rem;margin-bottom:16px}
  .pw-box h3{font-family:'DM Serif Display',serif;font-size:1.4rem;margin-bottom:8px}
  .pw-box p{font-size:0.88rem;color:#5a6882;margin-bottom:24px;line-height:1.5}
  .pw-input{width:100%;border:1.5px solid var(--border);border-radius:12px;padding:13px 16px;font-family:'DM Sans',sans-serif;font-size:1rem;outline:none;text-align:center;letter-spacing:0.15em;transition:border-color 0.18s,box-shadow 0.18s;margin-bottom:12px}
  .pw-input:focus{border-color:var(--eu-blue);box-shadow:0 0 0 3px rgba(0,51,153,0.08)}
  .pw-input.error{border-color:#b5290a;background:#fdecea;animation:shake 0.3s ease}
  @keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-8px)}75%{transform:translateX(8px)}}
  .pw-btn{width:100%;background:var(--eu-blue);color:white;border:none;border-radius:12px;padding:13px;font-family:'DM Sans',sans-serif;font-size:1rem;font-weight:600;cursor:pointer;transition:background 0.15s;margin-bottom:10px}
  .pw-btn:hover{background:#0040bb}
  .pw-cancel{background:none;border:none;color:#7a87a3;font-family:'DM Sans',sans-serif;font-size:0.88rem;cursor:pointer;padding:6px}
  .pw-cancel:hover{color:var(--ink)}
  .pw-error{font-size:0.82rem;color:#b5290a;margin-bottom:10px;display:none}
...
<div class="product-strip">
  <div class="product-strip-inner">
    ...
    <div class="admin-status">
      <button class="admin-badge locked" id="adminBtn" onclick="handleAdminBtn()">🔒 View Only</button>
    </div>
  </div>
</div>
...
<div class="grid-section">
  <div class="section-label" id="sectionLabel">16 DPP Data Categories — click any card to view</div>
...
<div class="pw-overlay" id="pwOverlay">
  <div class="pw-box">
    ...
    <h3>Admin Access</h3>
    <p>Enter the password to unlock editing mode.</p>
    <input type="password" class="pw-input" id="pwInput" placeholder="Enter password" onkeydown="if(event.key==='Enter')checkPassword()">
    <div class="pw-error" id="pwError">Incorrect password. Try again.</div>
    <button class="pw-btn" onclick="checkPassword()">Unlock Editing</button>
...
<script>
const CORRECT_PASSWORD = 'M1u2b3a4';
...
function checkPassword(){
  ...
  if(val === CORRECT_PASSWORD){
    ...
    btn.className = 'admin-badge unlocked';
    document.getElementById('sectionLabel').textContent = '16 DPP Data Categories — click any card to edit';
...
function lockAdmin(){
  ...
  btn.className = 'admin-badge locked';
  document.getElementById('sectionLabel').textContent = '16 DPP Data Categories — click any card to view';
...
function openModal(cat){
  ...
  // actions — edit buttons only for admin
  ...
  if(isAdmin){
    ...
    actions.innerHTML=`<button class="modal-close" onclick="closeModal()">✕</button>`;
  }

  // body — editable if admin, read-only if viewer
  const body=document.getElementById('modalBody');
  if(isAdmin){
    body.innerHTML=`<div class="modal-desc">${cat.desc}</div>`+
    ...
    body.innerHTML=`<div class="modal-desc">${cat.desc}</div>`+
      cat.fields.map((f,idx)=>{
        const val=saved[`f${idx}`]||'';
        return `<div class="field-view-group">
          <div class="field-view-label">${f.label}</div>
          <div class="field-view-value ${val?'':'empty'}">${val||'No data entered yet'}</div>
        </div>`;
      }).join('');
  }
...
function clearFields(){
  if(!currentCat||!isAdmin||!confirm('Clear all fields in this category?'))return;
  const d=loadData();
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    if(document.getElementById('pwOverlay').classList.contains('open')) closePwModal();
    else closeModal();
  }
  if(e.key==='Enter'&&document.getElementById('pwOverlay').classList.contains('open')) checkPassword();
});
